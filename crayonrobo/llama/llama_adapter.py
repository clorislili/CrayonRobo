...
class LLaMA_adapter(nn.Module):
    ...
    def direction_vector(self, loc1, loc2):
        import math
    ...
    def parallel_loss(self, predicted_vector, true_vector):
        predicted_vector = predicted_vector / torch.norm(predicted_vector)
        true_vector = true_vector / torch.norm(true_vector)

        cosine_similarity = torch.dot(predicted_vector, true_vector)

        loss = 1 - cosine_similarity

        return loss

    def forward(self, filename, tokens, labels, imgs,epoch,args):
        ...
        output = self.llama.output(h) #512x32000
        
        output = output[:, :-1, :] #511x32000
        ...
        output_decoded = str(self.tokenizer.decode(output_list))
        
        pattern_up = r'gripper up 3D direction is \[(-?\d+), (-?\d+), (-?\d+)\]'
        up_matches = re.findall(pattern_up, output_decoded)
        pattern_left = r'gripper left 3D direction is \[(-?\d+), (-?\d+), (-?\d+)\]'
        left_matches = re.findall(pattern_left, output_decoded)

        
        if args.ortho_loss == True and up_matches != [] and left_matches != []:
            up_matches_ortho = [[int(x)*0.02 + 0.0001 for x in tup] for tup in up_matches]
            left_matches_otrho = [[int(x)*0.02 + 0.0001 for x in tup] for tup in left_matches]

            vector1 = torch.tensor(up_matches_ortho[0], dtype=torch.float32)
            vector2 = torch.tensor(left_matches_otrho[0], dtype=torch.float32)
            dot_product = torch.dot(vector1, vector2)
            magnitude_product = torch.norm(vector1) * torch.norm(vector2)
            normalized_dot_product = (dot_product / magnitude_product + 1) / 2
            orthogonal_loss = 1-normalized_dot_product ** 2
            
        if args.para_loss == True and 'move_dir_hint4' in filename and up_matches != [] and left_matches != []:
            up_matches_para = [[int(x)*0.02 + 0.0001 for x in tup] for tup in up_matches]
            left_matches_para = [[int(x)*0.02 + 0.0001 for x in tup] for tup in left_matches]
            
            dir_path = os.path.dirname(filename[0])
            result_dir = os.path.join(dir_path,'result.json')
            with open(result_dir, 'r') as fin:
                data_inf = json.load(fin)
                camera_matrix = np.array(data_inf['camera_metadata']['camera_matrix'])
                x,y = data_inf['pixel_locs']
                point3d_cam = np.array(data_inf['position_cam'])
                
                up_point2 = point3d_cam - 0.13*np.array(up_matches_para[0])
                up_point2 = [-up_point2[1],-up_point2[2],up_point2[0]]
                up_xy1 = np.dot(camera_matrix[:3,:3],up_point2)
                up_xy = (up_xy1/up_xy1[2])[:2] # (x,y,r^2,r)
                up_vec_pred = torch.tensor(self.direction_vector((y,x),(up_xy[0],up_xy[1])), dtype=torch.float32)
                up_vec_gt = torch.tensor(data_inf['up_dir_new'], dtype=torch.float32)
                up_loss = self.parallel_loss(up_vec_pred,up_vec_gt)
                
                left_point2 = point3d_cam - 0.13*np.array(left_matches_para[0])
                left_point2 = [-left_point2[1],-left_point2[2],left_point2[0]]
                left_xy1 = np.dot(camera_matrix[:3,:3],left_point2)
                left_xy = (left_xy1/left_xy1[2])[:2] # (x,y,r^2,r)
                left_vec_pred = torch.tensor(self.direction_vector((y,x),(left_xy[0],left_xy[1])), dtype=torch.float32)
                left_vec_gt = torch.tensor(data_inf['left_dir_new'], dtype=torch.float32)
                left_loss = self.parallel_loss(left_vec_pred,left_vec_gt)
                parallel_loss = 0.5*up_loss + 0.5*left_loss
                

        if args.ortho_loss == True and args.para_loss == False and up_matches != [] and left_matches != []:
            loss = c_loss * 0.7 + orthogonal_loss * 0.3
        elif args.para_loss == True and 'move_dir_hint4' in filename and args.ortho_loss == False and up_matches != [] and left_matches != []:
            loss = c_loss * 0.7 + parallel_loss * 0.3
            
        elif args.ortho_loss == True and 'move_dir_hint4' in filename and args.para_loss == True and up_matches != [] and left_matches != []:
            loss = c_loss * 0.4 + parallel_loss * 0.3 + orthogonal_loss * 0.3
        else:
            loss = c_loss
