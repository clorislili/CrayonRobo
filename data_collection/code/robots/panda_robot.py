...
class Robot(object):
    ...
    def move_to_target_pose_cam(self, camera, target_ee_pose: np.ndarray, num_steps: int) -> None:
        """
        Move the robot hand dynamically to a given target pose
        Args:
            target_ee_pose: (4, 4) transformation of robot hand in robot base frame (ee2base)
        ...
        for i in range(num_steps):
            ...
            self.env.render()
            if i % 100 == 0:
                rgb_final_temp, _ = camera.get_observation()
                pic_list.append(rgb_final_temp)
        return pic_list
