...
class DataGen(object):
    ...
    def add_one_collect_job(self, data_dir, record_name):
        if self.is_running:
            printout(self.flog, 'ERROR: cannot add a new job while DataGen is running!')
            exit(1)

        todo = ('COLLECT', data_dir, record_name)
    ...
    def job_func(pid, todos, Q):
        ...
        for todo in todos:
            if todo[0] == 'COLLECT':
                cmd = 'xvfb-run -a python replay_data_traj.py --data_dir %s --record_name %s --no_gui' \
                        % (todo[1], todo[2])
                # print(cmd)
                # assert(0)
                folder_name = todo[1]
