...
parser.add_argument('--starting_epoch', type=int, default=400, help='if you want to run this data generation across multiple machines, you can set this parameter so that multiple data folders generated on different machines have continuous trial-id for easier merging of multiple datasets')
...
parser.add_argument('--ins_cnt_fn', type=str, default='category_count.txt', help='a file listing all category instance count, which is used to balance the interaction data amount to make sure that all categories have roughly same amount of data interaction, regardless of different shape counts in these categories')
...
print('-------all categories: ',conf.category_types)
